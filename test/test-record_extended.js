...
describe("Extended Record", () => {
  ...
  it("expect to have property fields, that is a non-empty object", () => {
    expect(recordExtended).to.have.property("fields").that.is.an("object").that
      .is.not.empty;
  });
  ...
  it("expect header to have 12 keys, +1 from parent", () => {
    expect(Object.keys(recordExtended.fields.header).length).to.equal(
      Object.keys(record.fields.header).length + 1
    );
  });
  it("expect body to have 4 keys, same as parent", () => {
    expect(Object.keys(recordExtended.fields.body).length).to.equal(
      Object.keys(record.fields.body).length
    );
  });
  ...
  it("expect header event ID to be 2 bytes, +1 from parent", () => {
    expect(recordExtended.fields.header.event_id).to.equal(
      record.fields.header.event_id + 1
    );
  });
