...
describe("Commander", () => {
  ...
  it("expect to handle invalid command IDs", () => {
    expect(() => new Commander("command")).to.throw(Error, "is not supported");
    expect(() => new Commander(9999)).to.throw(Error, "is not supported");
  });
  ...
  it("expect command setter to handle invalid commands", () => {
    expect(() => (commander_1.command = new Segment())).to.throw(
      Error,
      "must be an instance of Command"
    );
    expect(() => (commander_68.command = new Segment())).to.throw(
      Error,
      "must be an instance of Command"
    );
  });
