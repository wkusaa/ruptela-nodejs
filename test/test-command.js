...
describe("Command", () => {
  it("expect to handle invalid payload", () => {
    expect(() => new Command("payload")).to.throw(
      Error,
      "must be an instance of Segment"
    );
  });
  it("expect to handle unimplemented function 'execute'", () => {
    expect(() => new Command(new Segment())).to.throw(
      Error,
      "must implement function 'execute'"
    );
  });
});
...
describe("Command_1_68", () => {
  ...
  it("expect to have function 'execute'", () => {
    expect(command_1).to.have.property("execute").that.is.a("function");
    expect(command_68).to.have.property("execute").that.is.a("function");
    expect(command_1_68).to.have.property("execute").that.is.a("function");
  });
  it("expect to handle invalid payload", () => {
    expect(() => new Command_1_68("payload")).to.throw(
      Error,
      "must be an instance of Payload_1_68"
    );
  });
});
