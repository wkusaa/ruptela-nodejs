...
describe("Index", () => {
  ...
  it("expect to process valid input", () => {
    const buffer = Buffer.from(
      "0045000070473afaedd944000159fc5a40000000d3e542f6184a37820929465a0900000c00070100050005001d3a0c001e0ff0019300000192000000890000010041000004af004dc3",
      "hex"
    );
  ...
  it("expect to handle invalid input", () => {
    ...
    expect(response.error.message).to.have.string(
  ...
  it("expect to handle 'Command_1_68' input with invalid value sent for records left", () => {
    ...
    expect(response).to.have.property("error").that.is.an.instanceof(Error);
    expect(response.error.message).to.have.string(
      "Records left flag is not valid"
    );
  });
